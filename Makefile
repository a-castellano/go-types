PROJECT_NAME := "go-types"
PKG := "github.com/a-castellano/$(PROJECT_NAME)"
PKG_LIST := $(shell go list ${PKG}/... | grep -v /vendor/)
GO_FILES := $(shell find . -name '*.go' | grep -v /vendor/ | grep -v _test.go)

.PHONY: all build clean test test_rabbitmq_unit test_redis_unit test_slog_unit test_envelope_unit test_smtp_unit coverage coverhtml lint race msan help

all: build

lint: ## Lint the files
	@go vet ${PKG_LIST}

test: ## Run unit tests
	@go test --tags=unit_tests -short ./...

test_rabbitmq_unit: ## Run rabbitmq related tests
	@go test --tags=rabbitmq_unit_tests -short ./...

test_redis_unit: ## Run redis related tests
	@go test --tags=redis_unit_tests -short ./...

test_slog_unit: ## Run slog related tests
	@go test --tags=slog_unit_tests -short ./...

test_envelope_unit: ## Run envelope related tests
	@go test --tags=envelope_unit_tests -short ./...

test_smtp_unit: ## Run smtp related tests
	@go test --tags=smtp_unit_tests -short ./...

test_opentelemetry_unit: ## Run opentelemetry related tests
	@go test --tags=opentelemetry_unit_tests -short ./...

race: ## Run data race detector
	@go test --tags=unit_tests -race -short ./...

msan: ## Run memory sanitizer
	@CC=clang go test --tags=unit_tests -msan -short ./...

coverage: ## Generate global code coverage report
	./development/coverage.sh;

coverhtml: ## Generate global code coverage report in HTML
	go tool cover -html=cover/coverage.report -o coverage.html;

#build: ## Build the binary file
#	@go build -v $(PKG)

clean: ## Remove previous build
	@rm -f $(PROJECT_NAME)

help: ## Display this help screen
	@grep -h -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | awk 'BEGIN {FS = ":.*?## "}; {printf "\033[36m%-30s\033[0m %s\n", $$1, $$2}'

